...
(function (IdentifierType) {
    IdentifierType["CLASS"] = "class";
    IdentifierType["METHOD"] = "method";
    IdentifierType["VARIABLE"] = "variable";
    IdentifierType["CONSTANT"] = "constant";
    IdentifierType["PACKAGE"] = "package";
    IdentifierType["INTERFACE"] = "interface";
...
export class NamingUtils {
     ...
     * - 每个单词首字母大写
     */
    static checkPascalCase(name, type) {
     ...
     * - 除首字母外，每个单词首字母大写
     */
    static checkCamelCase(name, type) {
        const typeName = this.getTypeName(type);
        // 检查首字母是否小写
        if (!/^[a-z]/.test(name)) {
     ...
     */
    static checkConstantNaming(name) {
     ...
     * 检查包命名是否符合规范（全小写，点分隔）
     */
    static checkPackageNaming(name) {
     ...
     * 检查标识符中的缩写词使用是否规范
     */
    static checkAbbreviation(name, typeName) {
 ...
 * 常见的缩写词列表
 */
NamingUtils.COMMON_ABBREVIATIONS = [
