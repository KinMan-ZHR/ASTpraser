 ...
 */
export class VariableNamingRule {
    constructor() {
        this.id = 'backend/variable-naming';
        this.description = '检测Java变量和常量命名是否符合规范';
     ...
     * 递归查找所有变量声明
     */
    findVariableDeclarations(node, filePath, issues) {
        if (!node)
            return;
        // 检查是否是变量声明节点
        if (node.type === 'FieldDeclaration') {
            ...
            const isConstant = modifiers.some((mod) => (mod.keyword === 'static' && mod.keyword === 'final') ||
                mod.keyword === 'const');
            // 获取变量名
        ...
        if (node.type === 'LocalVariableDeclaration') {
            const modifiers = node.modifiers || [];
            const isConstant = modifiers.some((mod) => mod.keyword === 'final');
            // 遍历所有声明变量
...
//# sourceMappingURL=variable-naming.js.map