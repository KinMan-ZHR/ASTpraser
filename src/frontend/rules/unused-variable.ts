import { Issue, Rule, Severity, Location } from '../../common/types.js';

/**
 * 未使用变量检测规则
 */
export class UnusedVariableRule implements Rule {
  id = 'frontend/unused-variable';
  description = '检测未使用的变量声明';
   ...
   * 遍历AST找出所有变量声明
   */
  private findVariableDeclarations(nodes: any[], declarations: Map<string, any>): void {
   ...
   * 遍历AST找出所有变量使用
   */
  private findVariableUsages(nodes: any[], declarations: Map<string, any>): void {
    if (!nodes || !Array.isArray(nodes)) return;
