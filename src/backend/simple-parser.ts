...
export class SimpleJavaParser {
   ...
   * 查找所有类声明
   */
  private static findClasses(content: string): any[] {
    const classes = [];
   ...
   * 查找所有方法声明
   */
  private static findMethods(content: string): any[] {
    const methods = [];
   ...
   * 查找所有字段（变量）声明
   */
  private static findFields(content: string): any[] {
    const fields = [];
    // 匹配字段声明，包括静态和非静态字段
    ...
    while ((match = fieldRegex.exec(content)) !== null) {
      ...
      if (match[1]) modifiers.push({ keyword: match[1] });
      
      if (match[2]) {
        if (match[2].includes('static')) modifiers.push({ keyword: 'static' });
        if (match[2].includes('final')) modifiers.push({ keyword: 'final' });
      }
