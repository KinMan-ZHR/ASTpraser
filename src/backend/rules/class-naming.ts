 ...
 */
export class ClassNamingRule implements Rule {
  id = 'backend/class-naming';
  description = '检测Java类名是否正确遵循帕斯卡命名法';
   ...
   * 递归查找所有类声明
   */
  private findClassDeclarations(node: any, filePath: string, issues: Issue[]): void {
    if (!node) return;
